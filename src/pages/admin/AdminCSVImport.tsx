...
function parseCSV(text: string): { rows: MenuItemInput[]; errors: string[] } {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const validCategories = ['starters', 'main course', 'chatni', 'sweets', 'desserts', 'combo'];
    if (!validCategories.includes(row.category.toLowerCase())) {
      errors.push(`Row ${rowNum}: category must be Starters, Main Course, Chatni, Sweets, Desserts, or Combo (got "${row.category}")`);
      continue;
    }
...
function capitalizeCategory(c: string): string {
  const map: Record<string, string> = { 'starters': 'Starters', 'main course': 'Main Course', 'desserts': 'Desserts' };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
          <div>
            ...
            <ul className="space-y-0.5 text-blue-700">
              ...
              <li><code className="bg-blue-100 px-1 rounded">category</code> — Starters / Main Course / Desserts</li>
